...
public class FriendDaoImpl implements FriendDao{
	...
	public Friend getFriend(long userId, long friendId) {
		...
		UserDetails user=userDAO.getUserByUserId(userId);
		UserDetails friendUser=userDAO.getUserByUserId(friendId);
		Friend friend=(Friend)session.createQuery("from Friend where userId="+friendUser.getId()+" and friendId="+user.getId()).getSingleResult();
		return friend;
	}
